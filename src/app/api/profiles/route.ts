...
export async function GET() {
  ...
  const profiles = await profileRepo.listForUser(userId);
...
export async function POST(req: NextRequest) {
  ...
  const existing = await profileRepo.listForUser(userId);
  ...
  const profile = await profileRepo.create(userId, name.trim().slice(0, 40), color);
